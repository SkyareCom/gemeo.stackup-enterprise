(function(){
  ...
  function currentBountyValue(player){
    if(!player)return 0;
    if(Number.isFinite(+player.currentBounty))return Math.max(0,+player.currentBounty);
    return base.bountyEnabled()?Math.max(0,+state.bountyValue||0):0;
  }
  function awardBounty({winnerId,eliminatedId,value=null,staff=null,source='TOURNAMENT_ENGINE'}){
    ...
    const victimBounty=currentBountyValue(eliminated);
    const baseValue=value===null?victimBounty:Math.max(0,+value||0);
    if(baseValue<=0)return{ok:false,error:'ESTA OCORRÊNCIA NÃO POSSUI BOUNTY ATIVO.',code:'NO_ACTIVE_BOUNTY'};
    const pct=state.tournamentFormat==='PKO'?state.pkoCashPercent:100;
    const paid=baseValue*(pct/100),carry=baseValue-paid;
  ...
  function eliminatePlayer(args){
    ...
    const {playerId,staff,source='TOURNAMENT_ENGINE',killerId=null,skipBounty=false}=args||{};
    const p=base.activePlayer(playerId);
    if(!p)return{ok:false,error:'JOGADOR ATIVO NÃO ENCONTRADO.'};
    const activeBounty=base.bountyEnabled()?currentBountyValue(p):0;
    const bountyRequired=activeBounty>0&&!skipBounty&&!['PRESENCE_NO_SHOW','ADMIN_NO_SHOW','TOURNAMENT_CLOSE'].includes(source);
  ...
  function reenterPlayer(args){
    ...
    const p=base.playerById(args?.playerId);
    if(!p)return{ok:false,error:'JOGADOR NÃO ENCONTRADO.'};
    const out=originalReenter(args);
    if(!out.ok)return out;
