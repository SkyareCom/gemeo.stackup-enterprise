(function(){
  ...
  function tableCount(s,active){
    const explicit=n(s.activeTables||s.tableCount||s.tablesOpen);
    if(explicit)return explicit;
    const ids=new Set((active||activePlayers(s)).map(p=>String(p.table||'').trim()).filter(Boolean));
    return ids.size;
  }
  function fieldCount(s,m){
    const ids=new Set((s.seatCheckins||[]).filter(x=>x.eventId===s.eventId&&['SEATED','ELIMINATED'].includes(x.status)).map(x=>String(x.playerId)));
    return ids.size+m.re;
  }
  function totalChips(s,m){
    return m.buy*n(s.buyinChips||s.startingStack)+m.re*n(s.reentryChips||s.buyinChips||s.startingStack)+m.r1*n(s.rebuyChips)+m.r2*n(s.doubleRebuyChips)+m.a1*n(s.addonChips)+m.a2*n(s.specialAddonChips)+m.early*n(s.earlyBonusChips)+m.addonBonus*n(s.addonBonusChips);
  }
  function prizePool(s,m){
    let total=0;
    for(const t of m.tx){
      if(['PAYOUT','BOUNTY_PAYOUT','BONUS'].includes(t.type))continue;
      if(Number.isFinite(+t.breakdown?.prize)){total+=n(t.breakdown.prize);continue}
      if(t.type==='ENTRY')total+=n(s.buyin);
      else if(t.type==='REENTRY')total+=n(s.reentryValue||s.buyin);
      else if(item(t,'REBUY_I'))total+=n(s.rebuyValue);
      else if(item(t,'REBUY_II'))total+=n(s.doubleRebuyValue);
      else if(item(t,'ADDON_I'))total+=n(s.addonValue);
      else if(item(t,'ADDON_II'))total+=n(s.specialAddonValue);
      else if(t.type==='REBUY')total+=n(s.rebuyValue);
      else if(t.type==='DOUBLE_REBUY')total+=n(s.doubleRebuyValue);
      else if(t.type==='ADDON')total+=n(s.addonValue);
    }
    return total||n(s.prizePool);
  }
  ...
  function snapshot(s=state){
    const m=metrics(s),active=activePlayers(s),alternates=alternatePlayers(s),field=fieldCount(s,m),chips=totalChips(s,m),snap=window.TournamentTimer?.displayed?.()||s;
    return{m,active,alternates,left:active.length,field,chips,avg:active.length?chips/active.length:0,tables:tableCount(s,active),alternate:alternates.length,prize:prizePool(s,m),elapsed:elapsedSeconds(s,snap),snap};
  }
