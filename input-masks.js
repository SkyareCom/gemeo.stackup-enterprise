(function(){
  ...
  const prepare=el=>{
    ...
    if(!kind)return '';
    el.dataset.stackupMask=kind;el.inputMode='numeric';el.maxLength=kind==='cpf'?14:15;
    ...
    applyValue(el,kind);return kind;
  };
  ...
  const boot=()=>{
    scan(document);
    document.addEventListener('input',liveFormat,true);document.addEventListener('input',liveFormat,false);document.addEventListener('change',liveFormat,false);document.addEventListener('blur',liveFormat,true);
