...
function createManager(hostId,kind){
 ...
 function activate(group,noAlert=false){const id=selected[group];if(!id)return;api.toggle(kind,id,true);if(kind==='spoken'&&api.setSpokenNoAlert)api.setSpokenNoAlert(id,!!noAlert);selected[group]=null;open[group].activate=false;open[group].active=true;render()}
 ...
 function messageButton(id,isSelected,disabled,pickKind,group){return `<button type="button" class="msgPick ${isSelected?'selected':''}" data-msg-kind="${kind}" data-pick="${pickKind}" data-group="${group}" data-id="${id}" ${disabled?'disabled':''} aria-pressed="${isSelected?'true':'false'}"><span class="msgPickText"><b>${labelFor(id)}</b><small>${textFor(id)}</small></span></button>`}
 function block(g){
  const c=cfg(),rows=rowsFor(g.mode),activeRows=rows.filter(r=>!!c[kind]?.[r[0]]),availableRows=rows.filter(r=>!c[kind]?.[r[0]]),s=open[g.id];
  const spokenActions=kind==='spoken'?`<div class="msgPanelActions msgFourActions"><button type="button" data-msg-action="test" data-group="${g.id}" ${!selected[g.id]?'disabled':''}>TESTAR MENSAGEM</button><button type="button" data-msg-action="testNoAlert" data-group="${g.id}" ${!selected[g.id]?'disabled':''}>TESTAR MENSAGEM SEM ALERTA</button><button type="button" data-msg-action="confirm" data-group="${g.id}" ${!selected[g.id]?'disabled':''}>CONFIRMAR ATIVAÇÃO</button><button type="button" data-msg-action="confirmNoAlert" data-group="${g.id}" ${!selected[g.id]?'disabled':''}>ATIVAR MENSAGEM SEM ALERTA</button></div>`:`<div class="msgPanelActions"><button type="button" data-msg-action="test" data-group="${g.id}" ${!selected[g.id]?'disabled':''}>TESTAR MENSAGEM</button><button type="button" data-msg-action="confirm" data-group="${g.id}" ${!selected[g.id]?'disabled':''}>CONFIRMAR ATIVAÇÃO</button></div>`;
  const activateList=s.activate?`<div class="msgPanel">${availableRows.length?`<div class="msgList">${availableRows.map(r=>messageButton(r[0],selected[g.id]===r[0],false,'new',g.id)).join('')}</div>${spokenActions}`:`<div class="msgEmpty">TODAS AS MENSAGENS DESTE BLOCO JÁ ESTÃO ATIVAS.</div>`}</div>`:'';
  ...
  return `<section class="msgGroup"><div class="msgGroupTitle">${g.title}</div><div class="msgGroupButtons"><button type="button" data-msg-action="activateOpen" data-group="${g.id}">ATIVAR MENSAGEM</button><button type="button" data-msg-action="activeOpen" data-group="${g.id}">MENSAGENS ATIVAS${activeRows.length?' • '+activeRows.length:''}</button></div>${activateList}${activeList}</section>`
 }
 function bind(){
...
#spokenMessages,#writtenMessages{display:grid;gap:14px}.msgGroup{border:1px solid #27342D;border-radius:9px;background:linear-gradient(#0B100D,#060907);padding:10px}.msgGroupTitle{color:#8DFC3B;margin-bottom:8px}.msgGroupButtons,.msgPanelActions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.msgPanel{margin-top:10px;padding-top:10px;border-top:1px solid #27342D}.msgList{display:grid;gap:7px}.msgPick{display:block!important;width:100%!important;text-align:left!important;background:#020302!important;border:1px solid #27342D!important;padding:10px!important}.msgPick.selected{border-color:#8DFC3B!important;background:#0B100D!important}.msgPick:disabled{opacity:.7!important}.msgPickText{display:grid;gap:4px}.msgPickText b,#spokenMessages .msgPickText b,#writtenMessages .msgPickText b{color:#FFFFFF!important}.msgPickText small,#spokenMessages .msgPickText small,#writtenMessages .msgPickText small{color:#AEB8B1!important;line-height:1.35!important;font-weight:300!important}.msgPanelActions{margin-top:9px}.msgActiveHead{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:8px;color:#AEB8B1}.msgPanelActions .danger{border-color:#7a2b2b!important}.msgEmpty{color:#AEB8B1;padding:8px 2px}@media(max-width:700px){.msgGroupButtons,.msgPanelActions{grid-template-columns:1fr}.msgActiveHead{align-items:stretch;flex-direction:column}}
