...
function createManager(hostId,kind){
 ...
 function test(group){const id=selected[group]||activeSelected[group];if(!id)return;const c=cfg(),l=lang();if(kind==='spoken')api.playAlertThenSpeak(textFor(id),{lang:l,profileId:c.voiceProfile,repeat:c.voiceRepeat,volume:c.voiceVolume/100});else api.setAnnouncement(textFor(id))}
 function activate(group){const id=selected[group];if(!id)return;api.toggle(kind,id,true);selected[group]=null;open[group].activate=false;open[group].active=true;render()}
 ...
 function block(g){
  ...
  const activateList=s.activate?`<div class="msgPanel">${availableRows.length?`<div class="msgList">${availableRows.map(r=>messageButton(r[0],selected[g.id]===r[0],false,'new',g.id)).join('')}</div><div class="msgPanelActions"><button type="button" data-msg-action="test" data-group="${g.id}" ${!selected[g.id]?'disabled':''}>TESTAR MENSAGEM</button><button type="button" data-msg-action="confirm" data-group="${g.id}" ${!selected[g.id]?'disabled':''}>CONFIRMAR ATIVAÇÃO</button></div>`:`<div class="msgEmpty">TODAS AS MENSAGENS DESTE BLOCO JÁ ESTÃO ATIVAS.</div>`}</div>`:'';
