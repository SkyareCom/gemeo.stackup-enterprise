...
function block(g){
 const c=cfg(),rows=rowsFor(g.mode),activeRows=rows.filter(r=>!!c.spoken?.[r[0]]),availableRows=rows.filter(r=>!c.spoken?.[r[0]]),s=open[g.id];
