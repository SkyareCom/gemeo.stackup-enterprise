...
const looksUi=s=>{const t=clean(s);if(t.length<2||t.length>320)return false;if(/https?:\/\//i.test(t)||/[{}<>]=|function\b|const\b|let\b|var\b|=>|querySelector|classList|dataset|localStorage|Date\.|Math\.|JSON\.|document\.|window\./.test(t))return false;return hasTerm(t.toUpperCase(),sourceTerms)};
...
for(const file of files){const rel=path.relative(ROOT,file).replace(/\\/g,'/'),src=fs.readFileSync(file,'utf8');if(rel.endsWith('.html')){const hasLoader=/shared\.js|app-language\.js|input-masks\.js|ui-standard\.js|<iframe\b/i.test(src);if(!hasLoader)missingLoader.push(rel)}const strings=rel.endsWith('.html')?extractHtml(src):extractJs(src);for(const raw of strings){const text=clean(raw);if(!looksUi(text))continue;candidates++;const en=clean(i18n.translateString(text,'en')),es=clean(i18n.translateString(text,'es'));if(en===text||hasTerm(en.toUpperCase(),residualEn))failures.push(`${rel} :: EN :: ${text} => ${en}`);if(hasTerm(es.toUpperCase(),residualEs))failures.push(`${rel} :: ES :: ${text} => ${es}`)}}
