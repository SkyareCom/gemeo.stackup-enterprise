...
function extract(src){const out=[];let m;const textRx=/>\s*([^<>]+?)\s*</g;while((m=textRx.exec(src)))out.push(m[1]);const attrRx=/(?:placeholder|title|aria-label|value)\s*=\s*["']([^"']+)["']/gi;while((m=attrRx.exec(src)))out.push(m[1]);return out}
const failures=[];let candidates=0;
for(const file of walk(ROOT)){const rel=path.relative(ROOT,file).replace(/\\/g,'/'),src=fs.readFileSync(file,'utf8');for(const raw of extract(src)){const source=clean(raw);if(source.length<2||source.length>360||!/\p{L}/u.test(source))continue;if(/\$\{|=>|===|!==|\.html\b|\{[^}]*:|;|#[0-9A-F]{3,8}/i.test(source))continue;candidates++;const es=clean(i18n.translateString(source,'es'));const words=(es.toUpperCase().match(/[A-ZÁÉÍÓÚÜÑ-]+/g)||[]).filter(w=>suspicious.has(w)&&!allowed.has(w));if(words.length)failures.push(`${rel} :: ${source} => ${es} :: ENGLISH=[${[...new Set(words)].join(', ')}]`)}}
