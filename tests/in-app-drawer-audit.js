...
for(const required of ['stackup-select-trigger','stackup-select-list','MutationObserver','data-stackup-drawer',"font-family:'Caacupe One'",'border:0!important','background:transparent!important']){
  if(!listSource.includes(required))failures.push(`in-app-lists.js: falta padrão obrigatório ${required}`);
}
if(/Cantarell|Inter|Goldman|Arial,sans-serif/.test(listSource))failures.push('in-app-lists.js: contém fonte fora do padrão CAACUPE ONE');
