...
(async()=>{
 ...
 const now=Date.now();
 const state={setupTitleFirstV1:true,eventId:'',tournamentName:'QA SETUP',clubId:'club-a',clubName:'CLUBE A',activeEnvironmentId:'club-a',activeEnvironmentName:'CLUBE A',activeEnvironmentType:'CLUB',prepared:false,status:'',tournamentStatus:'',gameType:'NLH',tournamentFormat:'REGULAR',seatsPerTable:9,buyin:500,buyinChips:30000,startingStack:30000,fee:50,structure:[],savedStructures:[],
 ...
 try{
   ...
   const sb=row.locator('input[aria-label="SB"]');const bb=row.locator('input[aria-label="BB"]');const ante=row.locator('input[aria-label="ANTE"]');
   await sb.fill('100');await sb.blur();await bb.fill('200');await bb.blur();await ante.fill('200');await ante.blur();
   const ok=row.locator('[data-row-action="ok"],button').filter({hasText:'OK'}).first();await ok.click();
   ...
   assert('HISTÓRICO mostra estrutura salva',(await page.locator('#historyList').innerText()).includes('ESTRUTURA QA DINÂMICA'));
   await page.locator('#historyList .historyItem').filter({hasText:'ESTRUTURA QA DINÂMICA'}).getByRole('button',{name:'USAR'}).click();
   await page.waitForTimeout(100);
