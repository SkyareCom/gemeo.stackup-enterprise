(function(){
  ...
  const ROLE_ACCESS={
    DEALER:['DEALER_STATION'],
    FLOOR:['TOURNAMENTS','DEALER_STATION','FINAL_TABLE_ACCESS','PLAYERS','BALANCING','CHECKIN','CONTROL'],
    TD:['TOURNAMENTS','DEALER_STATION','FINAL_TABLE_ACCESS','PLAYERS','BALANCING','CHECKIN','CONTROL','RESULTS','SETUP','READINESS'],
    GESTOR:['*'],OWNER:['*']
  };
  const FLOOR_PAGES=new Set(['index.html','tournaments.html','dealer.html','final-table-access.html','players.html','players-directory.html','checkin.html','checkin-players.html','presence.html','balancing.html','control.html','tournament-players.html']);
  const TD_PAGES=new Set([...FLOOR_PAGES,'setup.html','ready-tournaments.html','tournament-readiness.html','tournament-close.html','bounty.html','screen.html','broadcast.html']);
  ...
  function ensure(){
    ...
    const clubId=state.clubId||('club-'+slug(state.clubName||'POKER CLUB'));
    state.clubId=clubId;
    let club=state.authClubs.find(c=>c.id===clubId);
    if(!club){club={id:clubId,name:state.clubName||'POKER CLUB',type:'CLUB',active:true,createdAt:Date.now()};state.authClubs.push(club)}
    (state.staffUsers||[]).forEach(s=>{
      ...
      s.personId=person.id;s.clubId=s.clubId||clubId;
      let m=state.authMemberships.find(x=>x.personId===person.id&&x.clubId===s.clubId);
      if(!m){m={id:'membership-'+s.id,personId:person.id,clubId:s.clubId,staffId:s.id,role:s.role||'DEALER',permissions:[...(s.permissions||ROLE_ACCESS[s.role]||[])],active:s.active!==false,createdAt:s.createdAt||Date.now()};state.authMemberships.push(m)}
  ...
  function pagePermission(path){const p=(path||location.pathname).split('/').pop().split('?')[0]||'index.html';const map={'dealer.html':'DEALER_STATION','final-table-access.html':'FINAL_TABLE_ACCESS','tournaments.html':'TOURNAMENTS','players.html':'PLAYERS','players-directory.html':'PLAYERS','checkin.html':'CHECKIN','control.html':'CONTROL','balancing.html':'BALANCING','setup.html':'SETUP','tournament-readiness.html':'READINESS','tournament-close.html':'RESULTS'};return map[p]||null}
  function guard(path){ensure();const p=(path||location.pathname).split('/').pop().split('?')[0]||'index.html';if(p==='login.html')return true;if(!state.authMemberships.length)return true;const s=current();if(!s){location.replace('login.html');return false}if(['GESTOR','OWNER'].includes(s.role))return true;if(s.role==='DEALER'){if(p!=='dealer.html'){location.replace('dealer.html');return false}return true}if(s.role==='FLOOR'&&!FLOOR_PAGES.has(p)){location.replace('index.html');return false}if(s.role==='TD'&&!TD_PAGES.has(p)){location.replace('index.html');return false}const req=pagePermission(p);if(req&&!can(req)){location.replace(landing(s));return false}return true}
