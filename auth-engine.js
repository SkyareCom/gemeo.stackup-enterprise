(function(){
  ...
  function current(){ensure();const s=getSession();if(!s)return null;const m=state.authMemberships.find(x=>x.id===s.membershipId&&x.active!==false);if(!m)return null;const club=state.authClubs.find(c=>c.id===s.clubId),permissions=permissionsFor(m.role,m.permissions);if(String(state.activeEnvironmentId||'')!==String(s.clubId||'')){syncActiveEnvironment(s.clubId,club);save()}return{...s,permissions,membership:{...m,permissions},person:state.authPeople.find(p=>p.id===s.personId),club}}
  ...
  function pagePermission(path){const p=(path||location.pathname).split('/').pop().split('?')[0]||'index.html';const map={'dealer.html':'DEALER_STATION','chip-count-ai.html':'CHIP_COUNT','smart-registration.html':'SMART_REGISTRATION','final-table-access.html':'FINAL_TABLE_ACCESS','tournaments.html':'TOURNAMENTS','tournament-manager.html':'TOURNAMENT_MANAGER','tournament-settings.html':'SETUP','players.html':'PLAYERS','players-hub.html':'PLAYERS','players-directory.html':'PLAYERS','player-register.html':'PLAYERS','players-registered.html':'PLAYERS','checkin.html':'CHECKIN','control.html':'CONTROL','balancing.html':'BALANCING','setup.html':'SETUP','tournament-readiness.html':'READINESS','tournament-close.html':'RESULTS'};return map[p]||null}
