...
function render(status=''){
 ...
 host.innerHTML=`<button type="button" class="alertMainButton" id="toggleAlertList">SELECIONAR ALERTAS</button>${open?`<div class="alertPanel"><div class="alertList">${A.PRESETS.map(row).join('')}</div><div class="alertConfig"><label class="alertControl"><span>PITCH</span><b id="alertPitchValue">${draft.pitch} ST</b><input id="alertPitch" type="range" min="-24" max="24" step="1" value="${draft.pitch}"></label><label class="alertControl"><span>REPETIÇÕES</span><b id="alertRepeatValue">${draft.repeats}X</b><input id="alertRepeat" type="range" min="1" max="8" step="1" value="${draft.repeats}"></label></div><div class="alertActions"><button type="button" id="memorizeAlert">MEMORIZAR PITCH E REPETIÇÕES</button><button type="button" id="testAlert">TESTAR</button><button type="button" id="activateAlert" class="${active?'active':''}">${active?'ALERTA ATIVADO':'ATIVAR ALERTA'}</button></div>${status?`<div class="alertStatus">${status}</div>`:''}</div>`:''}`;
 document.getElementById('toggleAlertList').onclick=()=>{open=!open;render()};
...
#alertManager{display:grid;gap:8px}.alertMainButton{width:100%!important}.alertPanel{margin-top:8px;border:1px solid #27342D;border-radius:9px;background:linear-gradient(#0B100D,#060907);padding:10px}.alertList{display:grid;gap:7px}.alertPick{display:grid!important;grid-template-columns:24px 1fr!important;align-items:center!important;gap:9px!important;width:100%!important;text-align:left!important;background:#020302!important;border:1px solid #27342D!important;padding:10px!important}.alertPick.selected{border-color:#8DFC3B!important}.alertSquare{width:20px;height:20px;border:1px solid #8DFC3B;border-radius:3px;display:grid;place-items:center;color:#8DFC3B}.alertConfig{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px}.alertControl{display:grid;grid-template-columns:1fr auto;gap:8px;padding:10px;border:1px solid #27342D;border-radius:9px}.alertControl input{grid-column:1/-1;width:100%;accent-color:#8DFC3B}.alertActions{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-top:10px}.alertActions button.active{background:#8DFC3B!important;color:#020302!important}.alertStatus{margin-top:8px;color:#8DFC3B}@media(max-width:700px){.alertConfig,.alertActions{grid-template-columns:1fr}}
