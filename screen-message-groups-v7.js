...
function createManager(hostId,kind){
 ...
 const activeButton=`<button type="button" class="activeMessagesButton" data-action="toggleActive" data-group="${g.id}">MENSAGENS ATIVAS${activeCount?` • ${activeCount}`:''}</button>`;
 return `<section class="msgGroup"><button type="button" class="msgCategoryButton active" data-category="${g.id}">${g.title}</button><div class="msgCategoryBody"><div class="msgList">${rows.map(r=>messageBlock(r,g,c)).join('')}</div>${activeButton}${activeList(g,c)}</div></section>`}
 function bind(){
