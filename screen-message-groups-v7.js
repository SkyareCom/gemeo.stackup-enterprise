...
function createManager(hostId,kind){
 ...
 const state={groups:Object.fromEntries(groups.map(g=>[g.id,{open:false,modes:Object.fromEntries(g.ids.map(id=>[id,REGISTRATION_IDS.has(id)?'noalert':'alert']))}])),showActive:false,editActive:false};
 const cfg=()=>api.load();
 ...
 function chooseGroup(id){const next=!state.groups[id].open;groups.forEach(g=>{state.groups[g.id].open=false});state.groups[id].open=next;render()}
 ...
 function removeActive(id){api.toggle(kind,id,false);render()}
 ...
 function block(g){const s=state.groups[g.id],c=cfg(),rows=rowsFor(g),activeCount=rows.filter(r=>!!c[kind]?.[r[0]]).length;if(!s.open)return `<button type="button" class="msgCategoryButton" data-category="${g.id}">${g.title}${activeCount?` • ${activeCount}`:''}</button>`;
 ...
 const list=count?rows.map(r=>`<div class="activeRow"><div class="msgText"><b>${labelFor(r[0])}</b><small>${textFor(r[0])}</small></div>${state.editActive?`<button type="button" class="deleteActiveButton" data-action="removeActive" data-id="${r[0]}">APAGAR</button>`:''}</div>`).join(''):'<div class="msgEmpty">NENHUMA MENSAGEM ATIVA.</div>';
 const editButton=count?`<button type="button" class="editActiveButton" data-action="toggleEdit">${state.editActive?'CONCLUIR EDIÇÃO':'EDITAR'}</button>`:'';
 return `<div class="activeFooter"><button type="button" class="activeMessagesButton active" data-action="toggleActive">MENSAGENS ATIVAS${count?` • ${count}`:''}</button><div class="activeList">${list}${editButton}</div></div>`}
 function bind(){
  host.querySelectorAll('[data-category]').forEach(b=>b.onclick=()=>chooseGroup(b.dataset.category));
  host.querySelectorAll('[data-action]').forEach(b=>b.onclick=e=>{e.stopPropagation();const a=b.dataset.action,g=b.dataset.group,id=b.dataset.id;if(a==='modeAlert')setMode(g,id,'alert');else if(a==='modeNoAlert')setMode(g,id,'noalert');else if(a==='test')test(g,id);else if(a==='confirm')confirm(g,id);else if(a==='toggleActive'){state.showActive=!state.showActive;if(!state.showActive)state.editActive=false;render()}else if(a==='toggleEdit'){state.editActive=!state.editActive;render()}else if(a==='removeActive')removeActive(id)})
 }
 function render(){const c=cfg();host.innerHTML=groups.map(block).join('')+activeFooter(c);bind()}
 render();return{render};
}
const style=document.createElement('style');style.textContent=`#spokenMessages,#writtenMessages{display:grid;gap:8px}.msgGroup{border:0!important;background:transparent!important;padding:0!important}.msgCategoryButton{width:100%!important;text-align:left!important;padding:12px!important}.msgCategoryButton.active{background:#0B100D!important;color:#fff!important}.msgCategoryBody{padding:10px 0 14px}.msgList{display:grid;gap:14px}.msgItem{padding:10px 0 14px;border:0!important;border-bottom:1px solid #18201B!important}.msgItem:last-child{border-bottom:0!important}.msgText{display:grid;gap:4px}.msgText b{color:#fff!important}.msgText small{color:#AEB8B1!important;line-height:1.35!important;font-weight:300!important}.msgText em{font-style:normal;color:#8DFC3B!important;font-weight:400!important}.msgPerActions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-top:10px}.msgPerActions button.active{background:#8DFC3B!important;color:#020302!important;border-color:#8DFC3B!important}.activeFooter{margin-top:14px;padding-top:6px}.activeMessagesButton,.editActiveButton{width:100%!important;margin-top:0!important}.activeMessagesButton.active{background:#0B100D!important;color:#fff!important}.activeList{display:grid;gap:8px;margin-top:10px}.activeRow{display:grid;grid-template-columns:1fr auto;gap:10px;align-items:center;padding:10px 0;border-bottom:1px solid #18201B}.activeRow:last-of-type{border-bottom:0}.editActiveButton{margin-top:8px!important}.deleteActiveButton{min-width:110px!important}.msgEmpty{color:#AEB8B1!important;padding:8px 2px}@media(max-width:700px){.msgPerActions{grid-template-columns:1fr}.activeRow{grid-template-columns:1fr}.deleteActiveButton{width:100%!important}}`;document.head.appendChild(style);
