...
function createManager(hostId,kind){
 ...
 const state={groups:Object.fromEntries(groups.map(g=>[g.id,{open:false,modes:Object.fromEntries(g.ids.map(id=>[id,REGISTRATION_IDS.has(id)?'noalert':'alert']))}])),showActive:false};
 const cfg=()=>api.load();
