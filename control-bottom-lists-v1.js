...
const txAmount=t=>{const paid=t?.payment?.totalAmount??t?.payment?.amount??t?.totalValue??t?.total??t?.value??0;return Math.max(0,num(paid))};
const defs={
...
PURCHASES:{label:'COMPRAS POR JOGADOR',kind:'purchases'}
};
const purchaseKinds=[
{code:'BI',label:'BUY IN',filter:t=>t.type==='ENTRY'},
{code:'RBI',label:'REBUY I',filter:t=>item(t)==='REBUY_I'||(!item(t)&&t.type==='REBUY')},
{code:'RBII',label:'REBUY II',filter:t=>item(t)==='REBUY_II'||(!item(t)&&t.type==='DOUBLE_REBUY')},
{code:'RE',label:'REENTRADA',filter:t=>t.type==='REENTRY'},
{code:'ADI',label:'ADD ON I',filter:t=>item(t)==='ADDON_I'},
{code:'ADII',label:'ADD ON II',filter:t=>item(t)==='ADDON_II'},
{code:'EB',label:'EARLY BONUS',filter:t=>item(t)==='EARLY_BONUS'||t.type==='EARLY_BONUS'},
{code:'AB',label:'ADD ON BONUS',filter:t=>item(t)==='ADDON_BONUS'||t.type==='ADDON_BONUS'}
];
let current='';
function css(){if($('controlBottomListsStyle'))return;const s=document.createElement('style');s.id='controlBottomListsStyle';s.textContent=`#controlBottomLists{margin-top:26px}.bottomListButtons{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.bottomListBtn{width:100%!important;min-height:44px!important;padding:10px!important;border:1px solid #8DFC3B!important;border-radius:9px!important;background:#060907!important;color:#8DFC3B!important}.bottomListBtn.active{background:#8DFC3B!important;color:#020302!important}.bottomListResult{display:grid;gap:8px;margin-top:10px}.bottomListRow{padding:10px;border:1px solid #27342D;border-radius:9px;background:#060907}.bottomListMeta{color:#AEB8B1;margin-top:4px;line-height:1.4}.purchaseTotal{color:#8DFC3B;margin-top:6px}.purchaseDetail{margin-top:5px;color:#fff;line-height:1.45}@media(max-width:700px){.bottomListButtons{grid-template-columns:1fr}}`;document.head.appendChild(s)}
...
function purchaseRows(){const rows=tx().filter(t=>purchaseKinds.some(k=>k.filter(t)));const grouped=new Map();for(const t of rows){const key=String(t.playerId||t.directoryId||t.cpf||t.playerName||'SEM_ID');let g=grouped.get(key);if(!g){g={player:playerById(t.playerId),name:t.playerName||'',cpf:t.playerCpf||t.cpf||'',table:t.table,seat:t.seat,total:0,items:{}};grouped.set(key,g)}g.total+=txAmount(t);for(const k of purchaseKinds){if(!k.filter(t))continue;const x=g.items[k.code]||(g.items[k.code]={label:k.label,count:0,total:0});x.count+=Math.max(1,+t.quantity||1);x.total+=txAmount(t);break}}
const arr=[...grouped.values()].sort((a,b)=>String(a.player?.name||a.name).localeCompare(String(b.player?.name||b.name),'pt-BR'));if(!arr.length)return '<div class="bottomListMeta">NENHUMA COMPRA REGISTRADA NESTE TORNEIO.</div>';return arr.map(g=>{const p=g.player||{name:g.name,cpf:g.cpf,table:g.table,seat:g.seat};const compact=purchaseKinds.map(k=>g.items[k.code]?`${String(g.items[k.code].count).padStart(2,'0')} ${k.code}`:'').filter(Boolean).join(' • ');const detail=purchaseKinds.map(k=>g.items[k.code]?`<div class="purchaseDetail">${String(g.items[k.code].count).padStart(2,'0')} ${esc(k.label)} • ${money(g.items[k.code].total)}</div>`:'').join('');return `<div class="bottomListRow"><b>${esc(p.name||'JOGADOR')}</b><div class="bottomListMeta">CPF ${esc(cpfOf(p)||g.cpf||'—')}</div><div class="bottomListMeta">COMPRAS: ${esc(compact||'—')}</div>${detail}<div class="purchaseTotal">VALOR GASTO: ${money(g.total)}</div></div>`}).join('')}
function render(){if(!$('controlBottomLists'))install();document.querySelectorAll('.bottomListBtn').forEach(b=>b.classList.toggle('active',b.dataset.list===current));const root=$('bottomListResult');if(!current){root.innerHTML='<div class="bottomListMeta">SELECIONE UMA LISTA.</div>';return}const d=defs[current];if(d.kind==='players'){const list=(state.players||[]).filter(d.filter);root.innerHTML=list.length?list.map(p=>playerRow(p)).join(''):'<div class="bottomListMeta">NENHUM JOGADOR NESTA LISTA.</div>'}else if(d.kind==='purchases')root.innerHTML=purchaseRows();else root.innerHTML=txRows(d)}
