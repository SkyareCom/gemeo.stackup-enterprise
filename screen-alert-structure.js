...
function render(){
 ...
 host.innerHTML=`
   ...
   <div class="soundPresetList ${alertsOpen?'open':''}">${A.PRESETS.map(p=>`<button type="button" class="soundPresetName ${cfg.preset===p[0]?'selected':''}" data-preset="${p[0]}">${p[1]}</button>`).join('')}</div>
