...
let ctx=null,active=[];
function load(){try{const raw=JSON.parse(localStorage.getItem(KEY)||'{}');delete raw.bass;delete raw.treble;const saved=Object.assign({},defaults,raw);if(saved.preset==='airportSoft')saved.preset='singleTone';saved.speed=Math.max(.5,Math.min(2,+saved.speed||1));return saved}catch(_){return {...defaults}}}
function save(v){const clean={preset:v.preset,pitch:+v.pitch||0,speed:Math.max(.5,Math.min(2,+v.speed||1)),repeats:Math.max(1,+v.repeats||1),volume:Math.max(0,Math.min(100,+v.volume||0))};localStorage.setItem(KEY,JSON.stringify(clean));window.dispatchEvent(new CustomEvent('stackup-alert-audio-change',{detail:clean}))}
