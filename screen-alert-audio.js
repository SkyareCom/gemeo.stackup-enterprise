...
const defaults={preset:'airport',pitch:0,repeats:1,volume:70};
let ctx=null,active=[];
function load(){try{const raw=JSON.parse(localStorage.getItem(KEY)||'{}');delete raw.bass;delete raw.treble;const saved=Object.assign({},defaults,raw);if(saved.preset==='airportSoft')saved.preset='singleTone';return saved}catch(_){return {...defaults}}}
function save(v){const clean={preset:v.preset,pitch:+v.pitch||0,repeats:Math.max(1,+v.repeats||1),volume:Math.max(0,Math.min(100,+v.volume||0))};localStorage.setItem(KEY,JSON.stringify(clean));window.dispatchEvent(new CustomEvent('stackup-alert-audio-change',{detail:clean}))}
